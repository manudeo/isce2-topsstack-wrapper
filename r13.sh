#!/usr/bin/env bash
# =============================================================================
# r13.sh — ISCE2 topsStack Step 13 wrapper
#
...
echo 'Start run 13'
cd $WORK_DIR/
sed -i 's/ &//g' run_files/run_13_generate_burst_igram
sed -i '/wait/d' run_files/run_13_generate_burst_igram
sed -i '/^[[:space:]]*$/d' run_files/run_13_generate_burst_igram

wc -l run_files/run_13_generate_burst_igram
mkdir run_13_generate_burst_igram
cd run_13_generate_burst_igram
split -l 1 ../run_files/run_13_generate_burst_igram R13

PROCESS_DIR=$WORK_DIR/run_13_generate_burst_igram
cd $WORK_DIR/
for filename in $WORK_DIR/run_13_generate_burst_igram/R*; do
  file2process=`basename $filename`
  date1=$(grep config_generate_igram_ $WORK_DIR/run_13_generate_burst_igram/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 4)
  date2=$(grep config_generate_igram_ $WORK_DIR/run_13_generate_burst_igram/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 5)
  ifg_path1=$WORK_DIR/interferograms/${date1}_${date2}/$IW_num
  ifg_path2=$WORK_DIR/interferograms/${date1}_${date2}/$IW_num1
  ifg_path3=$WORK_DIR/interferograms/${date1}_${date2}/$IW_num2
  if [[ -f ${ifg_path1}/fine_$last_burst_num.int.vrt && -f ${ifg_path1}/fine_$last_burst_num.int.vrt && -f ${ifg_path3}/fine_$last_burst_num2.int.vrt && -f ${ifg_path2}/fine_$last_burst_num1.int.vrt ]]; then
    echo "${date1}_${date2}: interferogram exists, skip to next"
    continue
      else
    echo "${date1}_${date2} interferogram does not exist."
  fi;
