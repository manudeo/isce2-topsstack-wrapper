#!/usr/bin/env bash
# =============================================================================
# r3.sh — ISCE2 topsStack Step 3 wrapper
#
...
echo 'Start run 03'
cd $WORK_DIR/
sed -i 's/ &//g' run_files/run_03_average_baseline
sed -i '/wait/d' run_files/run_03_average_baseline
sed -i '/^[[:space:]]*$/d' run_files/run_03_average_baseline

wc -l run_files/run_03_average_baseline
mkdir run_03_average_baseline
cd run_03_average_baseline
split -l 1 ../run_files/run_03_average_baseline R03

PROCESS_DIR=$WORK_DIR/run_03_average_baseline

cd $WORK_DIR/
for filename in $WORK_DIR/run_03_average_baseline/R*; do
  file2process=`basename $filename`
  single_date=$(grep config_baseline_ $WORK_DIR/run_03_average_baseline/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 3)
  date_pair=${reference_date}_${single_date}
  baseline_path=$WORK_DIR/baselines/$date_pair/
  if [[ -f ${baseline_path}/${date_pair}.txt ]]; then
  echo "${date_pair}.txt exists. skip."
  continue
  else
  echo "${date_pair}.txt does not exists."
  fi;
