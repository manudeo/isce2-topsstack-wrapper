#!/usr/bin/env bash
# =============================================================================
# r16.sh — ISCE2 topsStack Step 16 wrapper
#
...
echo 'Start run 16'
cd $WORK_DIR/
sed -i 's/ &//g' run_files/run_16_unwrap
sed -i '/wait/d' run_files/run_16_unwrap
sed -i '/^[[:space:]]*$/d' run_files/run_16_unwrap
wc -l run_files/run_16_unwrap
mkdir run_16_unwrap
cd run_16_unwrap
split -l 1 ../run_files/run_16_unwrap R16

PROCESS_DIR=$WORK_DIR/run_16_unwrap
cd $WORK_DIR/
for filename in $WORK_DIR/run_16_unwrap/R*; do
  file2process=`basename $filename`
  date_pair=$(grep config_igram_unw_ $WORK_DIR/run_16_unwrap/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 4,5)
  date_pair_path=$WORK_DIR/merged/interferograms/$date_pair
  if [ -f ${date_pair_path}/filt_fine.unw.xml ]; then
    echo "$date_pair: unwrapped interferogram exists, skip to next"
    continue
  else
  echo -n "$date_pair: "
    if [ -f ${date_pair_path}/filt_fine.int ]; then
      echo -n "filt_fine.int exists "
    fi;
    echo "filt_fine.unw does not exists, rerun"
  fi;
