#!/usr/bin/env bash
# =============================================================================
# r12.sh — ISCE2 topsStack Step 12 wrapper
#
...
echo 'Start run 12'
cd $WORK_DIR/
sed -i 's/ &//g' run_files/run_12_merge_reference_secondary_slc
sed -i '/wait/d' run_files/run_12_merge_reference_secondary_slc
sed -i '/^[[:space:]]*$/d' run_files/run_12_merge_reference_secondary_slc

wc -l run_files/run_12_merge_reference_secondary_slc
mkdir run_12_merge_reference_secondary_slc
cd run_12_merge_reference_secondary_slc
split -l 1 ../run_files/run_12_merge_reference_secondary_slc R12

PROCESS_DIR=$WORK_DIR/run_12_merge_reference_secondary_slc
cd $WORK_DIR/
for filename in $WORK_DIR/run_12_merge_reference_secondary_slc/R*; do
  file2process=`basename $filename`
  date1=$(grep config_merge_ $WORK_DIR/run_12_merge_reference_secondary_slc/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 3)
  path1=$WORK_DIR/merged/SLC/$date1
  path2=$WORK_DIR/merged/geom_reference/
  if [[ -f ${path1}/${date1}.slc.full.xml && -f ${path1}/${date1}.slc.full.vrt ]] || [[ -f ${path2}/${date1}.rdr.full.vrt && -f ${path2}/${date1}.rdr.full.xml ]]; then
    echo "${date1} : slc exists, skip to next "
    continue
      else
    echo "${date1} slc does not exist, rerun"
  fi;
