#!/usr/bin/env bash
# =============================================================================
# r14.sh — ISCE2 topsStack Step 14 wrapper
#
...
echo 'Start run 14'
cd $WORK_DIR/
sed -i 's/ &//g' run_files/run_14_merge_burst_igram
sed -i '/wait/d' run_files/run_14_merge_burst_igram
sed -i '/^[[:space:]]*$/d' run_files/run_14_merge_burst_igram
wc -l run_files/run_14_merge_burst_igram
mkdir run_14_merge_burst_igram
cd run_14_merge_burst_igram
split -l 1 ../run_files/run_14_merge_burst_igram R14

PROCESS_DIR=$WORK_DIR/run_14_merge_burst_igram
cd $WORK_DIR/
for filename in $WORK_DIR/run_14_merge_burst_igram/R*; do
  file2process=`basename $filename`
  date_pair=$(grep config_merge_igram_ $WORK_DIR/run_14_merge_burst_igram/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 4,5)
  date_pair_path=$WORK_DIR/merged/interferograms/$date_pair
  if [[ -f ${date_pair_path}/fine.int.xml && -f ${date_pair_path}/fine.int && -f ${date_pair_path}/fine.int.vrt ]]; then
    echo "$date_pair: fine.int.xml exists, skip to next"
    continue
  else
  echo -n "$date_pair: "
    echo "fine.int.xml does not exists, rerun"
  fi;
