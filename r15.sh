#!/usr/bin/env bash
# =============================================================================
# r15.sh — ISCE2 topsStack Step 15 wrapper
#
...
echo 'Start run 15'
cd $WORK_DIR/
sed -i 's/ &//g' run_files/run_15_filter_coherence
sed -i '/wait/d' run_files/run_15_filter_coherence
sed -i '/^[[:space:]]*$/d' run_files/run_15_filter_coherence
wc -l run_files/run_15_filter_coherence
mkdir run_15_filter_coherence
cd run_15_filter_coherence
split -l 1 ../run_files/run_15_filter_coherence R15

PROCESS_DIR=$WORK_DIR/run_15_filter_coherence
cd $WORK_DIR/
for filename in $WORK_DIR/run_15_filter_coherence/R*; do
  file2process=`basename $filename`
  date_pair=$(grep config_igram_filt_coh_ $WORK_DIR/run_15_filter_coherence/$file2process | rev | cut -d / -f 1 | rev | cut -d _ -f 5,6)
  date_pair_path=$WORK_DIR/merged/interferograms/$date_pair
  if [[ -f ${date_pair_path}/filt_fine.int && -f ${date_pair_path}/filt_fine.int.xml && -f ${date_pair_path}/filt_fine.cor && -f ${date_pair_path}/filt_fine.cor.xml ]]; then
    echo "$date_pair: filt_fine.int and filt_fine.cor exist, skip to next"
    continue
  else
  echo -n "$date_pair: "
    echo "filt_fine.int does not exists, rerun"
  fi;
